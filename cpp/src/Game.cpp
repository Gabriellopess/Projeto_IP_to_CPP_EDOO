...
void Game::spawn_initial_collectibles() {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        for (const auto& c : coletaveis_) usedY.push_back(c->rect().y);

        std::vector<int> heights(std::begin(kHeights), std::end(kHeights));
        std::vector<int> available;
        for (int h : heights) {
            if (std::find(usedY.begin(), usedY.end(), h) == usedY.end()) available.push_back(h);
        }
        rect.y = available.empty() ? heights[static_cast<std::size_t>(random_int(0, (int)heights.size() - 1))]
                                   : available[static_cast<std::size_t>(random_int(0, (int)available.size() - 1))];

        // x: respeitar distância mínima
        int attempts = 0;
        while (true) {
            rect.x = random_int(cfg_.largura_tela + kSpawnMinXOffset, cfg_.largura_tela + kSpawnMaxXOffset);
            bool ok = true;
...
void Game::respawn_coletavel(std::size_t idx) {
    if (idx >= coletaveis_.size()) throw std::out_of_range("respawn_coletavel idx");

    // Troca o tipo aleatoriamente
    const auto type = random_choice_tipo_coletavel();

    // tenta altura diferente
    std::vector<int> usedY;
...
TipoColetavel Game::random_choice_tipo_coletavel() {
    ...
    if (r == 0) return TipoColetavel::Banana;
    if (r == 1) return TipoColetavel::Camera;
...
void Game::acelerar() {
    if (cfg_.acelerar_a_cada_ticks <= 0) return;
...
void Game::checar_finalizando() {
    ...
    if (!finalizando_ && distancia_metros >= (cfg_.meta_metros - cfg_.offset_finalizacao_metros)) {
        finalizando_ = true;
        ticks_finalizando_ = 0;
        std::cout << "[FINALIZANDO] reached " << distancia_metros << "m, entering final walk...\n";
    }
}
...
void Game::log_tick_header() const {
    const int distancia_metros = distancia_pixels_ / cfg_.pixels_por_metro;
    std::cout << "tick=" << tick_ << " dist=" << distancia_metros << "m " << contadores_;
    if (efeitos_.flash_ativo()) std::cout << " flash(" << efeitos_.flash_restante() << ")";
    if (finalizando_) std::cout << " [finalizando]";
    std::cout << "\n";
}

void Game::log_pickup(const Coletavel& c) const {
    std::cout << "  pickup: " << c.nome() << " (" << to_string(c.tipo()) << ")\n";
}

GameResult Game::rodar_simulacao() {
    // Simulação fixa: em alguns ticks, força pulo só pra exemplificar
    ...
    while (true) {
        ...
        // win condition: final walk terminou
        if (finalizando_ && ticks_finalizando_ >= cfg_.duracao_andar_final_ticks) {
                std::cout << "=== Result: WIN (finished runway) ===\n";
